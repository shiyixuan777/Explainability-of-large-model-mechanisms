...
| Locate | 完成 domain/prompt sweep、focused capital probe、PCA、error analysis | `figures/probe_sweep.csv`；`figures/probe_capital_answer.csv`；`figures/pca_capital_layer8.png` |
| Steer & Improve | 完成 held-out probe-direction steering，并诚实报告 without output improvement | `figures/steering_capital_probe_layer8.csv`；`scripts/run_steering.py` |
...
4. Activation patching 是 related capital recall 的补充因果实验，不是 true/false verification 的直接 patching。
5. Probe-direction steering 使用 group held-out split；`alpha=0` held-out probe accuracy 为 0.826，而不是全量拟合后的 1.000。
6. Steering 可以移动内部 probe score，但 without output improvement。
