...
|---|---:|---:|---:|---:|
| 全位置干预 | +0.133 | [0.094, 0.239] | [0.092, 0.233] | 0.625 |
| 仅提示词末位置| +0.135 | [0.096, 0.239] | [0.090, 0.232] | 0.625 |
| 仅补全文本内部位置 | -0.002 | [-0.003, 0.004] | [-0.004, 0.008] | 0.625 |
...
本文以 GPT-2-small 在事实验证任务中的高探针 AUC 为切入点，系统考察了该现象是否能够被解释为稳定的抽象真值表征。实验表明，原始首都事实数据中的高可分性受到明显词汇伪线索影响；在通过词汇平衡消除实体边际频率和线性词袋线索后，第 6 层最后词元位置的残差激活仍保留中等强度的标签可分性。将该方向迁移至直接补全提示的第 6 层提示末词元位置进行干预，可以稳定改变“正确首都—选定错误首都”的补全得分差，并在重复划分中持续超过所采样的随机方向和标签置乱方向。

然而，这一干预效应主要体现为补全评分的连续变化，而非稳定的事实选择纠正。配对偏好准确率仅有轻微改善，候选集中的正确首都排名虽有所提升，但第一名准确率仍然较低；同时，正确与错误补全的对数概率均受到较大程度的共同提升。因此，现有证据更支持一种有限结论：GPT-2-small 在词汇平衡首都事实中存在与事实配对和补全兼容度相关的可读信号，该信号具有一定的干预效力，但尚不足以被视为跨领域稳定、能够直接控制模型输出的全局真值方向，也不能代表完整的事实判断机制。

按照定位、干预与改进的行动式机制可解释性框架 [12]，本文完成了信号的层级读出定位和推理时方向干预，并对行为改进的范围进行了系统检验。研究结果说明，机制可解释性中的“可读出”“可干预”和“可改善”是三个不同层次：高探针性能不能自动推出模型自然使用该方向，评分变化也不能自动推出离散选择或生成行为得到可靠改善。本文在 Bao et al. [1] 所关注的真值方向泛化问题基础上，通过词汇平衡、长度归一化、随机与标签置乱对照、重复划分和消融分析，进一步揭示了线性标签信号的有效范围与解释边界。
